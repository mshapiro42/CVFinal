...
def findAccidentals(img):
    if img.shape[1] > 45:
        ...
        tempN = cv2.imread('Images/natural.jpg')
...
def notesTest(img_name):
    ...
    object_img = np.copy(notes)
    object_img = cv2.cvtColor(object_img,cv2.COLOR_GRAY2BGR)
    for i, o in enumerate(objects):
        cv2.rectangle(object_img, (o[0], o[1]), (o[2], o[3]), (255, 0, 0))
    # cv2.imshow("Black Connected Components",object_img)
    # cv2.waitKey()
    # cv2.imwrite("concomponents.jpg",object_img)
...
trebChrom = ['C4#', 'D4', 'D4#', 'E4', 'F4', 'F4#', 'G4', 'G4#', 'A4', 'A4#', 'B4', 'C5', 'C5#', 'D5', 'D5#', 'E5',
             'F5', 'F5#', 'G5', 'G5#']
bassChrom = ['E2', 'F2', 'F2#', 'G2', 'G2#', 'A2', 'A2#', 'B2', 'C3', 'C3#', 'D3', 'D3#', 'E3', 'F3', 'F3#', 'G3',
             'G3#', 'A3', 'A3#', 'B3', 'C4']

eighth1 = cv2.imread("Images/eighth-1.jpg")
eighth2 = cv2.imread("Images/eighthtail2-1.jpg")
quarter1 = cv2.imread("Images/quarter-1.jpg")
quarter2 = cv2.imread("Images/quarter2-1.jpg")
half1 = cv2.imread("Images/half-1.jpg")
half2 = cv2.imread("Images/half2-1.jpg")
whole = cv2.imread("Images/whole-1.jpg")
...
if __name__ == '__main__':
    for i in range(1, 7):
        name = 'Images\mySimpleSong' + str(i) + '.jpg'
        img, durations, notes = notesTest(name)
        name2 = 'Images\mySimpleSong' + str(i) + '-solved.jpg'
